...
if [[ ${BASELINE_CHECK} == "true" ]]; then
  ...
  ${project_binary_dir}/test/compare.py ./${prefix}/restarts/vector/ufs_land_restart.${YY}-${MM}-${DD}_${HH}-00-00.nc ${TEST_BASEDIR}/ufs_land_restart_anal.${YY}-${MM}-${DD}_${HH}-00-00.nc ${TOL}
  if [[ $? != 0 ]]; then
