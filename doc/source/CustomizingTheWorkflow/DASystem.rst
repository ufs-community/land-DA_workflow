...
The :jedi:`geometry <using/building_and_running/config_content.html#geometry>` section is used in JEDI configuration files "to define the model grid (both horizontal and vertical) and its parallelization across compute nodes." Most geometry definitions in this section are borrowed from the FV3-JEDI :jedi:`Geometry <inside/jedi-components/fv3-jedi/classes.html#geometry>`, :jedi:`FieldMetadata <inside/jedi-components/fv3-jedi/classes.html#fieldmetadata>`, and :jedi:`State/Increment/Field <inside/jedi-components/fv3-jedi/classes.html#state-increment-fields>` documentation. Note that for variational DA (e.g., 3D-Var data assimilation), the ``geometry:`` section appears in :jedi:`multiple places <inside/jedi-components/mpas-jedi/classes.html#nml-file-and-streams-file>` --- under ``cost function:`` and within each of the ``iterations:`` vector members under ``variational:``.
