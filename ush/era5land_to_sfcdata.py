...
def main():
    ...
    work_dir = "./"
    fn_oro_base = "C96_oro_data.tile"
    fn_oro_ext = ".nc"
    fn_sfc_base = "20221223.000000.sfc_data.tile"
    fn_sfc_ext = ".nc"
    plot_src_data = "YES"
    plot_sfc_data = "YES"
    BG_EXT_SRC_OPT = "gfs"  # "era5land" or "gfs"
    ...
    sde_cs_max = 25
    sde_cs_min = 0
    bg_ext_src_opt_upper = BG_EXT_SRC_OPT.upper()
    out_title_src_base = f'''Land-DA::{bg_ext_src_opt_upper}::'''
    out_fn_src_base = f'''landda_{BG_EXT_SRC_OPT}_src_'''
    out_title_sfc_base = f'''Land-DA::sfc_data::'''
    out_fn_sfc_base = f'''landda_{BG_EXT_SRC_OPT}_sfc_'''

    # Set the path to Natural Earth dataset
    cartopy_ne_path = "/work2/noaa/epic/chjeon/landda_test_hercules/land-DA_workflow/fix/NaturalEarth"
    cartopy.config['data_dir']=cartopy_ne_path

    # --- Read data from source data file ---
    if BG_EXT_SRC_OPT == "era5land":
        ds1 = xr.open_dataset("era5_land_20221223_data_0.nc")  # source data
    ...
    elif BG_EXT_SRC_OPT == "gfs":
        ds1 = xr.open_dataset("gfs.t00z.sfcanl.nc")  # source data
        ...
        print(f'''{bg_ext_src_opt_upper}::sea-land-ice  - max: {np.nanmax(ds1_land)}, min: {np.nanmax(ds1_land)}, shape: {ds1_land.shape}''')
    ...
    print(f'''{bg_ext_src_opt_upper}::2D::latitude      - max: {np.nanmax(ds1_lat)}, min: {np.nanmin(ds1_lat)}, shape: {ds1_lat.shape}''')
    print(f'''{bg_ext_src_opt_upper}::2D::longitude     - max: {np.nanmax(ds1_lon)}, min: {np.nanmin(ds1_lon)}, shape: {ds1_lon.shape}''')
    print(f'''{bg_ext_src_opt_upper}::Snow Depth        - max: {np.nanmax(ds1_sde)}, min: {np.nanmin(ds1_sde)}, shape: {ds1_sde.shape}''')
    print(f'''{bg_ext_src_opt_upper}::Soil Moisture (1) - max: {np.nanmax(ds1_swvl1)}, min: {np.nanmin(ds1_swvl1)}, shape: {ds1_swvl1.shape}''')
    print(f'''{bg_ext_src_opt_upper}::Soil Moisture (2) - max: {np.nanmax(ds1_swvl2)}, min: {np.nanmin(ds1_swvl2)}, shape: {ds1_swvl2.shape}''')
    print(f'''{bg_ext_src_opt_upper}::Soil Moisture (3) - max: {np.nanmax(ds1_swvl3)}, min: {np.nanmin(ds1_swvl3)}, shape: {ds1_swvl3.shape}''')
    print(f'''{bg_ext_src_opt_upper}::Soil Moisture (4) - max: {np.nanmax(ds1_swvl4)}, min: {np.nanmin(ds1_swvl4)}, shape: {ds1_swvl4.shape}''')
    ...
    for it in range(num_tiles):
        ...
        print(f'''SFC_DATA:: Tile {itp}:: latitude  - max: {np.nanmax(ds2_lat)}, min: {np.nanmin(ds2_lat)}, shape: {ds2_lat.shape}''')
        print(f'''SFC_DATA:: Tile {itp}:: longitude - max: {np.nanmax(ds2_lon)}, min: {np.nanmin(ds2_lon)}, shape: {ds2_lon.shape}''')
        ...
        print(f'''SFC_DATA:: Tile {itp}:: Snow Depth          - max: {np.nanmax(ds3_snwdph)}, min: {np.nanmin(ds3_snwdph)}, shape: {ds3_snwdph.shape}''')
        print(f'''SFC_DATA:: Tile {itp}:: Soil Moisture (1)   - max: {np.nanmax(ds3_smc1)}, min: {np.nanmin(ds3_smc1)}, shape: {ds3_smc1.shape}''')
        print(f'''SFC_DATA:: Tile {itp}:: Soil Moisture (2)   - max: {np.nanmax(ds3_smc2)}, min: {np.nanmin(ds3_smc2)}, shape: {ds3_smc2.shape}''')
        print(f'''SFC_DATA:: Tile {itp}:: Soil Moisture (3)   - max: {np.nanmax(ds3_smc3)}, min: {np.nanmin(ds3_smc3)}, shape: {ds3_smc3.shape}''')
        print(f'''SFC_DATA:: Tile {itp}:: Soil Moisture (4)   - max: {np.nanmax(ds3_smc4)}, min: {np.nanmin(ds3_smc4)}, shape: {ds3_smc4.shape}''')
        ...
        print(f'''Regridded SFC_DATA:: Tile {itp}:: Soil Moisture (1) - max: {np.nanmax(ds3_smc1_new)}, min: {np.nanmin(ds3_smc1_new)}, shape: {ds3_smc1_new.shape}''')
        print(f'''Regridded SFC_DATA:: Tile {itp}:: Soil Moisture (2) - max: {np.nanmax(ds3_smc2_new)}, min: {np.nanmin(ds3_smc2_new)}, shape: {ds3_smc2_new.shape}''')
        print(f'''Regridded SFC_DATA:: Tile {itp}:: Soil Moisture (3) - max: {np.nanmax(ds3_smc3_new)}, min: {np.nanmin(ds3_smc3_new)}, shape: {ds3_smc3_new.shape}''')
        print(f'''Regridded SFC_DATA:: Tile {itp}:: Soil Moisture (4) - max: {np.nanmax(ds3_smc4_new)}, min: {np.nanmin(ds3_smc4_new)}, shape: {ds3_smc4_new.shape}''')
        ...
        print(f'''New 3-D smc:: shape: {smc_new_3d.shape}''')

        smc_new_4d = np.broadcast_to(smc_new_3d, ds3_smc_all4.shape)
        print(f'''New 4-D smc:: shape: {smc_new_4d.shape}''')
        print(f'''New 4-D smc:: Tile {itp}:: Soil Moisture (1) - max: {np.nanmax(smc_new_4d[0,0,:,:])}, min: {np.nanmin(smc_new_4d[0,0,:,:])}''')
        print(f'''New 4-D smc:: Tile {itp}:: Soil Moisture (2) - max: {np.nanmax(smc_new_4d[0,1,:,:])}, min: {np.nanmin(smc_new_4d[0,1,:,:])}''')
        print(f'''New 4-D smc:: Tile {itp}:: Soil Moisture (3) - max: {np.nanmax(smc_new_4d[0,2,:,:])}, min: {np.nanmin(smc_new_4d[0,2,:,:])}''')
        print(f'''New 4-D smc:: Tile {itp}:: Soil Moisture (4) - max: {np.nanmax(smc_new_4d[0,3,:,:])}, min: {np.nanmin(smc_new_4d[0,3,:,:])}''')

        # --- Replace NaN values with zeros
        print(f'''SFC_DATA:: original:: No. of NaNs = {np.isnan(ds3_smc_all4).sum()}''')
        print(f'''SFC_DATA:: original:: No. of zeros = {np.count_nonzero(ds3_smc_all4 == 0)}''')
        print(f'''SFC_DATA:: regridded (before):: No. of NaNs = {np.isnan(smc_new_4d).sum()}''')
        print(f'''SFC_DATA:: regridded (before):: No. of zeros = {np.count_nonzero(smc_new_4d == 0)}''')

        smc_new_4d_no_nan = np.nan_to_num(smc_new_4d, nan=0.0)
        ds3["smc"][:] = smc_new_4d_no_nan

        print(f'''SFC_DATA:: regridded (after):: No. of NaNs = {np.isnan(ds3["smc"].values).sum()}''')
        print(f'''SFC_DATA:: regridded (after):: No. of zeros = {np.count_nonzero(ds3["smc"].values == 0)}''')

        chk_smc_new = ds3["smc"].values
        print(f'''Check smc:: shape: {chk_smc_new.shape}''')
        print(f'''Check smc:: Tile {itp}:: Soil Moisture (1) - max: {np.nanmax(chk_smc_new[0,0,:,:])}, min: {np.nanmin(chk_smc_new[0,0,:,:])}''')
        print(f'''Check smc:: Tile {itp}:: Soil Moisture (2) - max: {np.nanmax(chk_smc_new[0,1,:,:])}, min: {np.nanmin(chk_smc_new[0,1,:,:])}''')
        print(f'''Check smc:: Tile {itp}:: Soil Moisture (3) - max: {np.nanmax(chk_smc_new[0,2,:,:])}, min: {np.nanmin(chk_smc_new[0,2,:,:])}''')
        print(f'''Check smc:: Tile {itp}:: Soil Moisture (4) - max: {np.nanmax(chk_smc_new[0,3,:,:])}, min: {np.nanmin(chk_smc_new[0,3,:,:])}''')

        # --- Save new dataset ---
        fn_sfc_new = f'''{fn_sfc_base}{itp}_{BG_EXT_SRC_OPT}{fn_sfc_ext}'''
